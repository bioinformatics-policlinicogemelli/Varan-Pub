...
def create_meta_study(cancer, project_name, description, output_dir,version):
    """
...
def meta_case_main(cancer,output_folder):
    ...
    if os.path.exists(cases_list_dir):
        logger.info("It seems that this folder already exists")
        pass
    else:
    ...
    create_meta_study(cancer,project, description, output_folder,version)
    
    if os.path.exists(os.path.join(output_folder,"data_clinical_patient.txt")):
        create_meta_clinical_patient(cancer, project_name, output_folder,version)
        logger.info("meta_clinical_patient.txt created!")
    if os.path.exists(os.path.join(output_folder,"data_clinical_sample.txt")):
        create_meta_clinical_sample(cancer,project_name, output_folder,version)
    ...
    if os.path.exists(os.path.join(output_folder,"data_mutations_extended.txt")):
        ...
        if file_extended.shape[0]>1:
            populate_cases_sequenced(cancer,project_name,output_folder,cases_list_dir,version,logger)
        else:
    ...
    if os.path.exists(os.path.join(output_folder,"data_cna.txt")):
        ...
        if file_cna.shape[0]>1:
            populate_cases_cna(cancer, project_name,output_folder, cases_list_dir,version,logger)
        else:
    ...
    if os.path.exists(os.path.join(output_folder,"data_sv.txt")):
        ...
        if file_sv.shape[0]>1:
            populate_cases_sv(cancer,project_name, output_folder,cases_list_dir,version,logger)
        else:
